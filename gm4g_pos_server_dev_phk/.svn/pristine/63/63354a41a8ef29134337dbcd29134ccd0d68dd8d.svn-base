...
(function($) {
	...
	$.modal.defaults = {
		opacity: 0,			//	Modified by Kwan, changed from 50
	...
	$.modal.impl = {
			...
			if(ie6) {
				this.dialog.iframe = $('<iframe src="javascript:false;"/>')
										.css($.extend(this.opts.iframeCss, {
											display: 'none',
											opacity: 0,
											position: 'absolute',		// Modified by Kwan, changed from "fixed"
											height: w[0],
											width: w[1],
		...
		getDimensions: function() {
			...
			var w = el.width() > this.opts.minScreenWidth ? el.width() : this.opts.minScreenWidth;		//	Modified by Kwan, set the width not less than the min. screen width
		...
		setPosition: function() {
			...
			if (left < 0) left = 0;		// Modified by Kwan
			if (top < 0) top = 0;		// Modified by Kwan
